...
public class PostBO {
	...
	private static final int POST_MAX_SIZE = 3; // 스펙이 변경되거나 할 경우 여기서 바꿈
	...
	public List<Post> getPostListByUserId(int userId, Integer prevId, Integer nextId) {
		...
		if (prevId != null) {  // 이전
			...
			List<Post> postList = postDAO.getPostListByUserId(userId, direction, standardId, POST_MAX_SIZE);
		...
		// 첫페이지일 때(페이징X) standareId, direction이 null
		// 다음일 때 standardId, direction 채워져서 넘어감
		return postDAO.getPostListByUserId(userId, direction, standardId, POST_MAX_SIZE);
	}
